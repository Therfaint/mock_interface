...
class ParamDefine extends React.Component {
    ...
    constructor(props){
        ...
        this.columns = [{
            ...
            render: (text, record, index) => (
                <div>
                    <Icon type="minus-circle-o" style={{fontSize: 18, marginLeft: 3, cursor: 'pointer'}}  onClick={()=> this.onDelete(record)}/>
                    <Icon type="plus-circle-o" style={{fontSize: 18, marginLeft: 5, cursor: 'pointer'}} onClick={()=> this.handleAdd(record)}/>
                </div>
            )
        }];
        this.state = {
